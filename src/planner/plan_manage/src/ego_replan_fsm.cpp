...
void EGOReplanFSM::init(ros::NodeHandle& nh)
{
    ...
    broadcast_ploytraj_sub_ = nh.subscribe<traj_utils::PolyTraj>("planning/broadcast_traj_recv", 100, &EGOReplanFSM::RecvBroadcastPolyTrajCallback, this,
...
void EGOReplanFSM::checkCollisionCallback(const ros::TimerEvent& e)
{
    ...
    for (double t = t_cur; t < info->duration; t += time_step)
    {
        ...
        for (size_t id = 0; id < planner_manager_->traj_.swarm_traj.size(); id++)
        {
            ...
            if (dist < CLEARANCE)
            {
                ROS_WARN("swarm distance between drone %d and drone %d is %f, too close!", planner_manager_->pp_.drone_id, id, dist);
                t_temp = t;
                occ    = true;
                break;
            }
...
void EGOReplanFSM::waypointCallback(const geometry_msgs::PoseStampedPtr& msg)
{
    ...
    success =
        planner_manager_->planGlobalTrajWaypoints(odom_pos_, odom_vel_, Eigen::Vector3d::Zero(), one_pt_wps, Eigen::Vector3d::Zero(), Eigen::Vector3d::Zero());
...
void EGOReplanFSM::planGlobalTrajbyGivenWps()
{
    ...
    bool success = planner_manager_->planGlobalTrajWaypoints(odom_pos_, Eigen::Vector3d::Zero(), Eigen::Vector3d::Zero(), wps, Eigen::Vector3d::Zero(),
                                                             Eigen::Vector3d::Zero());
...
bool EGOReplanFSM::callReboundReplan(bool flag_use_poly_init, bool flag_randomPolyTraj, bool use_formation)
{
    ...
    bool plan_success =
        planner_manager_->reboundReplan(desired_start_pt, desired_start_vel, desired_start_acc, desired_start_time, local_target_pt_, local_target_vel_,
                                        (have_new_target_ || flag_use_poly_init), flag_randomPolyTraj, use_formation, have_local_traj_);
