...
mat jacobianF(const colvec& X, const colvec& U, double dt)
{
    ...
    F(0, 5) =
        -(dt * dt * (ay * (cos(ro) * cos(ya) + sin(pt) * sin(ro) * sin(ya)) - az * (cos(ya) * sin(ro) - cos(ro) * sin(pt) * sin(ya)) + ax * cos(pt) * sin(ya)))
        / 2;
    F(0, 6) = dt;
    F(0, 7) = 0;
    F(0, 8) = 0;
    ...
    F(1, 5) =
        (dt * dt * (az * (sin(ro) * sin(ya) + cos(ro) * cos(ya) * sin(pt)) - ay * (cos(ro) * sin(ya) - cos(ya) * sin(pt) * sin(ro)) + ax * cos(pt) * cos(ya)))
        / 2;
    F(1, 6) = 0;
    F(1, 7) = dt;
    F(1, 8) = 0;
    ...
    F(3, 4) = dt
              * ((wz * (cos(pt - ro) / 2 + cos(pt + ro) / 2)) / cos(pt) - (wy * (sin(pt - ro) / 2 - sin(pt + ro) / 2)) / cos(pt)
