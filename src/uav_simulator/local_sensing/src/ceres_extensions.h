...
template <typename T> inline void EigenQuaternionToScaledRotation(const T q[4], T R[3 * 3])
{
...
template <typename T> inline void EigenQuaternionToRotation(const T q[4], T R[3 * 3])
{
...
template <typename T> inline void EigenUnitQuaternionRotatePoint(const T q[4], const T pt[3], T result[3])
{
    const T t2 = q[3] * q[0];
    const T t3 = q[3] * q[1];
    const T t4 = q[3] * q[2];
...
template <typename T> inline void EigenQuaternionRotatePoint(const T q[4], const T pt[3], T result[3])
{
    // 'scale' is 1 / norm(q).
...
template <typename T> inline void EigenQuaternionProduct(const T z[4], const T w[4], T zw[4])
{
