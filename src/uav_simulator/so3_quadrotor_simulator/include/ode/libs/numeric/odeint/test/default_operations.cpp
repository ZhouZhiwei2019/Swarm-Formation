...
template <class Value> struct internal_value_type
{
    typedef Value type;
};
template <class T> struct internal_value_type<std::complex<T>>
{
    typedef T type;
};

template <class T> struct default_eps;
template <> struct default_eps<double>
{
...
template <> struct default_eps<float>
{
...
template <class Value, class Compare = typename internal_value_type<Value>::type> struct double_fixture
{
...
template <class Value, class Compare = typename internal_value_type<Value>::type> struct unit_fixture
{
    typedef Value                                   value_type;
    typedef Compare                                 compare_type;
    ...
    unit_fixture(const compare_type& eps_ = default_eps<compare_type>::def_eps())
        : m_eps(eps_), res(0.0 * si::meter), x(1.0 * si::meter), d1x(2.0 * si::meter / si::second), d2x(3.0 * si::meter / si::second / si::second)
    {
    }
