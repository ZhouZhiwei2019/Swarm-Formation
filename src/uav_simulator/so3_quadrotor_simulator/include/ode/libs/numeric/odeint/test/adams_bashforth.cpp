...
struct lorenz
{
    template <class State, class Deriv, class Value> void operator()(const State& _x, Deriv& _dxdt, const Value& dt) const
    {
...
template <class State> class rk4_decorator
{
public:
    size_t do_count;
    ...
    template <class System, class StateInOut, class DerivIn> void do_step(System system, StateInOut& x, const DerivIn& dxdt, value_type t, value_type dt)
    {
...
BOOST_AUTO_TEST_CASE(test_auto_initialization)
{
    ...
    adams_bashforth<3, state_type, value_type, state_type, value_type, range_algebra, default_operations, initially_resizer, rk4_decorator<state_type>> adams;
