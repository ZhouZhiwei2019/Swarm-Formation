...
/* reset dispatcher */
template <class StepperCategory> struct resetter
{
    template <class Stepper> static void reset(Stepper& stepper) {}
};

template <> struct resetter<explicit_error_stepper_fsal_tag>
{
    template <class Stepper> static void reset(Stepper& stepper)
    {
        stepper.reset();
...
template <class Stepper> struct perform_runge_kutta_test
{
    void operator()(void)
    {
...
template <class Stepper> struct perform_runge_kutta_error_test
{
    void operator()(void)
    {
        Stepper   stepper;
...
typedef mpl::vector<runge_kutta_cash_karp54_classic<state_type>, runge_kutta_cash_karp54<state_type>, runge_kutta_dopri5<state_type>,
                    runge_kutta_fehlberg78<state_type>>
