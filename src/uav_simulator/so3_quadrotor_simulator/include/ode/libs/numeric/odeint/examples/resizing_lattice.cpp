...
struct compacton_lattice
{
    ...
    void energy_distribution(const coord_type& q, const coord_type& p, coord_type& energies)
    {
        ...
        for (size_t i = 0; i < N; i++)
        {
            ...
            energies[i]       = p[i] * p[i] / 2.0 + m_pot[m_pot_start_index + i] * q[i] * q[i] * q[i] * q[i] / 4.0 + m_beta / 4.0 * diff * diff * diff * diff;
            en += energies[i];
        }
        en = 1.0 / en;
        for (size_t i = 0; i < N; i++)
    ...
    double energy(const coord_type& q, const coord_type& p)
    {
        ...
        {
            const double diff = q[(i + 1) % N] - q[i];
            en += p[i] * p[i] / 2.0 + m_pot[m_pot_start_index + i] * q[i] * q[i] * q[i] * q[i] / 4.0 + m_beta / 4.0 * diff * diff * diff * diff;
        }
        return en;
    }
...
int main()
{
    ...
    typedef symplectic_rkn_sb3a_mclachlan<coord_type, coord_type, double, coord_type, coord_type, double, range_algebra, default_operations, always_resizer>
                                    hamiltonian_stepper;
